...
function TOOL:LeftClick( trace )
    if SERVER then
        ...
        if self.TargetEntity.superpaints and #self.TargetEntity.superpaints >= maxpaints then 
        ...
        DoSuperPaint(self.TargetEntity, self:GetClientInfo("mat"), trace.HitPos, trace.HitNormal, self:GetClientNumber("width"), self:GetClientNumber("height"))
        EmitSound("superpaintsploink.mp3", trace.HitPos, 0, CHAN_STATIC, 1, 100)
    end
end
...
if CLIENT then
    ...
    function TOOL.BuildCPanel( panel )
        ...
        panel:ControlHelp("Material Lists - Lists for unmounted games will not show")
